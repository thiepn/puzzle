...
for(const f of ['app.js','word-dictionary.js','word-content.js','sw.js','scripts/validate-word-content.js','scripts/test-word-entry.mjs','scripts/test-accessibility-controls.mjs','scripts/test-phase10-integration.mjs','scripts/test-sensory-feedback.mjs','scripts/test-learning-onboarding.mjs','scripts/test-difficulty-quality.mjs','scripts/test-variety-quality.mjs','scripts/test-generator-stress.mjs','scripts/test-completion-quality.mjs','scripts/test-player-fuzz.mjs']){const r=run('node',['--check',f]);must(r.status===0,`${f}: syntax failed: ${r.stderr.trim()}`);}
...
notes.push(`content errors: ${contentSummary?.errors ?? 'unknown'}`);notes.push('playable games: 36');notes.push(`service worker: ${expected.cache}`);notes.push('Phase 13 difficulty/quality contract: enabled');notes.push('Phase 14 variety/anti-repetition contract: enabled');notes.push('Phase 15 generator robustness contract: enabled');notes.push('Phase 16 completion certification contract: enabled');notes.push('Phase 17 player state-fuzz contract: enabled');
