...
const docsIntegrityTest=run('node',['scripts/test-docs-integrity.mjs']);must(docsIntegrityTest.status===0,`documentation integrity regression failed: ${(docsIntegrityTest.stderr||docsIntegrityTest.stdout).trim()}`);
const ciHygieneTest=run('node',['scripts/test-ci-hygiene.mjs']);must(ciHygieneTest.status===0,`CI/workflow hygiene regression failed: ${(ciHygieneTest.stderr||ciHygieneTest.stdout).trim()}`);
const realDeviceReadinessTest=run('node',['scripts/test-real-device-readiness.mjs']);must(realDeviceReadinessTest.status===0,`real-device readiness regression failed: ${(realDeviceReadinessTest.stderr||realDeviceReadinessTest.stdout).trim()}`);
...
notes.push(`content errors: ${contentSummary?.errors ?? 'unknown'}`);notes.push('playable games: 36');notes.push(`service worker: ${expected.cache}`);notes.push('Phase 13 difficulty/quality contract: enabled');notes.push('Phase 14 variety/anti-repetition contract: enabled');notes.push('Phase 15 generator robustness contract: enabled');notes.push('Phase 16 completion certification contract: enabled');notes.push('Phase 17 player state-fuzz contract: enabled');notes.push('Phase 18 cross-browser/PWA/multi-tab resilience contract: enabled');notes.push('Phase 19 performance/memory/endurance contract: enabled');notes.push('Phase 20 recovery/deployment/maintenance certification contract: enabled');notes.push('QoL keyboard/mobile certification contract: enabled');notes.push('Maintenance handoff contract: enabled');notes.push('Documentation integrity contract: enabled');notes.push('CI/workflow hygiene contract: enabled');notes.push('Real-device readiness contract: enabled; physical closure remains evidence-driven');
