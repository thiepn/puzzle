...
if(['1.11.0','1.12.0'].includes(version)){
...
notes.push(`content errors: ${contentSummary?.errors ?? 'unknown'}`);notes.push('playable games: 36');notes.push(`service worker: ${expected.cache}`);notes.push('Phase 13 difficulty/quality contract: enabled');notes.push('Phase 14 variety/anti-repetition contract: enabled');notes.push('Phase 15 generator robustness contract: enabled');notes.push('Phase 16 completion certification contract: enabled');notes.push('Phase 17 player state-fuzz contract: enabled');notes.push('Phase 18 cross-browser/PWA/multi-tab resilience contract: enabled');notes.push('Phase 19 performance/memory/endurance contract: enabled');
