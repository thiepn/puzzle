...
test('precache uses v24 and scoped cache key',async()=>{const h=harness();await h.fire('install');const keys=[...h.stores.keys()];assert.equal(keys.length,1);assert.match(keys[0],/puzzle-arcade-core-.*v24$/);assert.equal(h.stores.get(keys[0]).size,9);});
...
test('activate only deletes same-scope old caches',async()=>{const h=harness();await h.fire('install');const own=[...h.stores.keys()][0];h.stores.set(own.replace(/v24$/,'v17'),new Map());h.stores.set('puzzle-arcade-core-%2Fother%2F-v17',new Map());await h.fire('activate');assert.ok(!h.stores.has(own.replace(/v24$/,'v17')));assert.ok(h.stores.has('puzzle-arcade-core-%2Fother%2F-v17'));assert.equal(h.claimed,1);});
